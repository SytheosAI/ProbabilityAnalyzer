...
-- Designed for PostgreSQL 14+ with TimescaleDB extension for time-series optimization
-- This schema supports all modules in the sports betting analysis system
...
-- Create daily partitions for odds (example)
CREATE TABLE odds_history_2024_01_01 PARTITION OF odds_history 
    FOR VALUES FROM ('2024-01-01') TO ('2024-01-02');
-- Continue creating daily partitions...

-- Create TimescaleDB hypertable for better time-series performance
...
CREATE TABLE line_movements_2024_01_01 PARTITION OF line_movements 
    FOR VALUES FROM ('2024-01-01') TO ('2024-01-02');
...
-- Schedule maintenance tasks (using pg_cron or similar)
-- SELECT cron.schedule('clean-cache', '0 * * * *', 'CALL clean_expired_cache();');
-- SELECT cron.schedule('refresh-views', '*/15 * * * *', 'CALL refresh_materialized_views();');
