...
-- Supabase-compatible version (pgvector removed, using JSONB for embeddings)
...
CREATE TABLE IF NOT EXISTS public.players (
    ...
    embedding_vector JSONB, -- Store embeddings as JSONB array
...
CREATE TABLE IF NOT EXISTS public.player_tracking_data (
    ...
    movement_tensor JSONB, -- Store tensor data as JSONB
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS public.line_movements (
    ...
    trigger_type VARCHAR(50), -- 'sharp_action', 'injury_news', 'weather', 'public_heavy'
...
CREATE TABLE IF NOT EXISTS public.social_sentiment (
    id BIGSERIAL PRIMARY KEY,
    entity_type VARCHAR(20), -- 'team', 'player', 'game'
    ...
    text_embeddings JSONB, -- BERT embeddings as JSON array
...
CREATE TABLE IF NOT EXISTS public.news_impact (
    ...
    news_type VARCHAR(50), -- 'injury', 'trade', 'suspension', 'personal'
    headline TEXT,
    source VARCHAR(100),
    credibility_score DECIMAL(3,2),
    impact_score DECIMAL(3,2), -- -1 to 1 impact on performance
...
CREATE TABLE IF NOT EXISTS public.coaches (
    ...
    team_id VARCHAR(100),
    sport VARCHAR(20),
    role VARCHAR(50), -- 'head', 'offensive', 'defensive'
    years_experience INTEGER,
    career_win_percentage DECIMAL(5,2),
    playoff_win_percentage DECIMAL(5,2),
    championships INTEGER DEFAULT 0,
    aggressive_index DECIMAL(3,2), -- 0 to 1
...
CREATE TABLE IF NOT EXISTS public.head_to_head_ml (
    ...
    game_flow_tensor JSONB, -- Historical game flow patterns
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.weather_conditions (
    ...
    passing_impact_score DECIMAL(3,2), -- -1 to 1
...
CREATE TABLE IF NOT EXISTS public.model_performance_context (
    ...
    sport VARCHAR(20),
    context_type VARCHAR(50), -- 'home_favorite', 'road_dog', 'primetime', 'division'
...
CREATE TABLE IF NOT EXISTS public.feature_importance (
    ...
    feature_type VARCHAR(50), -- 'numeric', 'categorical', 'embedding'
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.gpu_training_batches (
    ...
    model_type VARCHAR(50), -- 'ensemble', 'neural_net', 'xgboost', 'transformer'
    sport VARCHAR(20),
    batch_size INTEGER,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS public.model_performance_summary AS
SELECT 
    m.model_id,
    m.sport,
...
CREATE POLICY "model_perf_read" ON public.model_performance_context 
    FOR SELECT TO authenticated USING (true);
...
CREATE POLICY "model_perf_service" ON public.model_performance_context 
    FOR ALL TO service_role USING (true);
...
-- Insert sample GPU batch configuration for NVIDIA 5090
INSERT INTO public.gpu_training_batches (
    ...
    'ensemble',
    'ALL',
    1024,  -- Optimal batch size for 5090
    250,   -- Number of features
    100000,
    20000,
    20000,
    24.0,  -- 5090 has 24GB VRAM
    0.001,
    '{
        "optimizer": "AdamW",
...
BEGIN
    ...
    IF table_count = 19 AND view_count = 3 AND function_count = 3 THEN
        ...
        RAISE NOTICE '✅ NVIDIA 5090 GPU configuration added';
    ELSE
        RAISE WARNING '⚠️  Some components may be missing';
        RAISE WARNING '⚠️  Please check for errors above';
    END IF;
